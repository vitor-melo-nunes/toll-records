...
# Create SQS Topic
docker exec -it localstack aws --endpoint-url=http://localhost:4566 sqs create-queue --queue-name toll-records-created-notifications-pending-queue --region sa-east-1

# Subscriber SQS in SNS
docker exec -it localstack aws --endpoint-url=http://localhost:4566 sns subscribe --topic-arn arn:aws:sns:sa-east-1:000000000000:toll-records-created --protocol sqs --notification-endpoint arn:aws:sqs:sa-east-1:000000000000:toll-records-created-notifications --region sa-east-1
